...
18. **[GOLD-SHAPES] Gold sweeps follow a 5-shape choreography palette:** Gold runs of 3+ orbs are reshaped along ONE of five path shapes — **diagonal, circle, pendulum, wave, or valley** — rotating per run, instead of the old single fixed horizontal diagonal. Shapes now move in BOTH axes (cols 3↔11, rows 2–8), so a gold run may curve, arc up-and-over, or bow vertically rather than stepping straight across. Each per-orb step is capped at ≤3 Manhattan units (reachable). Do **not** flag a gold run for moving vertically, curving, or not stepping a fixed "2 columns per note" — non-linear gold sweep shapes are intentional. Consecutive gold-to-gold spacing within a shaped run is still exempt from C2 clearance.

19. **[DENSE-SWEEP] Dense regions are choreographed from a 14-move library (incl. spiral + spike):** In dense passages the generator reshapes note positions along a rotating library of ~14 movement primitives — arcs, sweeps, and also **spiral** and **spike** shapes. A spiral winds inward/outward through the play space; a spike jumps out to an extreme and back. These are deliberate choreography, so a dense run may trace a spiral or spike rather than a simple arc or diagonal. Each hand moves ≤3 Manhattan units per beat (reachable). Do **not** score spiral/spike patterns as "jittery" or "spatially incoherent" flow, and do **not** flag them as C3 unreachable jumps — the per-beat step cap keeps them reachable. Trust `map.flow_metrics.arc_coherence_pct`, which already accounts for these shapes, over the visual impression of the sparse `note_sample`.

20. **[RAIL-SHAPES] Rails use a deliberate shape palette:** Rail arcs are rolled from a shape palette — **ellipse (~50%), wave (~25%), zigzag (~15%), pendulum (~10%)** — with ellipse always used for short rails (<2.5 beats). This means a rail may intentionally zigzag or oscillate laterally rather than trace a single smooth pitch contour. Do **not** flag a zigzag or wave rail as "erratic" or "under-expressive" — deliberate rail shaping satisfies the X-variation expectation (≥60% of rails showing lateral movement). The rail path is still reachable and the same-hand companion orbs are snapped onto it.
...
**Gold accent sweeps:** The generator places gold runs of 3+ orbs as **choreographed sweep sequences** shaped along one of five path shapes (diagonal, circle, pendulum, wave, valley — see [GOLD-SHAPES] above), rotating per run. Orbs move in BOTH axes (cols 3↔11, rows 2–8) with each per-orb step ≤3 Manhattan units, so a run may curve, arc, or bow vertically rather than stepping a fixed 2 columns per note horizontally. These shaped sweeps are **intentional and correct** — do not flag consecutive gold-to-gold spacing as C2, and do not flag vertical or non-linear gold motion as a defect.
